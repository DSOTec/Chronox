...
const PlanCard = ({ price, planName, features, buttonText = "Get started", isPopular = false }) => {
  return (
    <div
      className={`relative flex flex-col bg-white rounded-xl border-2 md:p-8 p-6 transition-all duration-300 sm:shadow-[#24cdae6d] hover:shadow-lg border-[#24CDAF] hover:border-[#1fb89d]`}
    >
      {isPopular && (
        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
          <span className="bg-[#24CDAF] text-white px-4 py-1 rounded-full text-sm font-medium">Most Popular</span>
        </div>
      )}
      ...
      <div className="md:space-y-4 space-y-2 md:mb-8 mb-4">
        {features.map((feature, index) => (
          <div key={index} className="flex items-start">
            ...
            <span className={`text-gray-700 ${!feature.included ? "line-through text-gray-400" : ""}`}>{feature.text}</span>
          </div>
        ))}
      </div>

      <button className="w-full max-w-[200px] md:self-center bg-[#24CDAF] hover:bg-[#1fb89d] text-white font-medium py-3 px-6 rounded-[30px] transition-colors duration-200">
        {buttonText}
      </button>
    </div>
  );
